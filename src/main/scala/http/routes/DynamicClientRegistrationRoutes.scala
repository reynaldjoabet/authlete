...
/**
  ...
  * requests via GET, PUT, and DELETE to {@code /api/register/client_id}, where {@code client_id} is
  * the client ID of the registered client. This implementation will parse the client ID from the
